...
int get_shortest_distance_index(vector<double> x_pts, vector<double> y_pts, double x_pos, double y_pos){
  int index = 0;
  double dist_min;       
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          double steer_output;
          int index_shortest;
          index_shortest = get_shortest_distance_index(x_points, y_points, x_position, y_position);

          /**
          * TODO (step 3): compute the steer error (error_steer) from the position and the desired trajectory
          **/
          double angular_error = angle_between_points(x_position, y_position, x_points[index_shortest], y_points[index_shortest]) - yaw;
          double positional_error = y_points[index_shortest] - y_position;
          error_steer = angular_error + positional_error;
          cout << "Angular error: " << angular_error << ", Positional error: " << positional_error << endl;
          ...
          **/
          ...
          error_throttle = v_points[index_shortest] - velocity;
